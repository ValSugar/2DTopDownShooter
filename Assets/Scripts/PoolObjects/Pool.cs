...
namespace PoolObjects
{
	public class Pool : MonoBehaviour
	{
		...
		public static T SpawnObject<T>(T transtormObject, Vector3 position, Quaternion rotation) where T : MonoBehaviour
		{
			var result = Instance.SpawnObject(transtormObject.gameObject, position, rotation);
			return result.GetComponent<T>();
		}

		private GameObject SpawnObject(GameObject poolObject, Vector3 position, Quaternion rotation)
		{
			var pool = GetOrCreatePool(poolObject);
			...
			{
				result = Instantiate(poolObject, position, rotation, transform);
			}
